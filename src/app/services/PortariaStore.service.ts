...
import { PortariaApiService } from "./PortariaApi.service";
import {
  VehicleResponseDto,
  VehicleResponseDtoStatusEnum,
  ControleVeiculosControllerFindAllQueryParamsStatusEnum
} from "@/api/portaria";
import { Observable, from, map, catchError, of, tap, combineLatest, Subject } from "rxjs";
...
const VIDEO_GATEWAY_ORIGIN = new URL(BASE_URL).origin;
...
export class PortariaStoreService extends SignalStore<any> {
  ...
  listarVeiculos(query: any): Observable<any> {
    ...
    return this.portariaApi.listarVeiculos(params).pipe(
      map((response: any) => ({
        ...
        page: (response?.page !== undefined) ? response.page + 1 : 1,
  ...
  getVideoSession(): Observable<{ manifestUrl: string }> {
    ...
    return from(fetch(`${BASE_URL}/video/session`, {
      method: 'POST',
      headers: {
  ...
  private normalizeVideoUrl(url?: string): string {
    if (!url) return '';
    try {
      return new URL(url, VIDEO_GATEWAY_ORIGIN).toString();
    } catch {
      return url;
    }
  }
  ...
  public mapToVeiculoDTO(v: VehicleResponseDto): VeiculoDTO {
    const constructedFoto = (v as any).foto ? `${BASE_URL}/veiculos/foto/${(v as any).foto}` : undefined;
